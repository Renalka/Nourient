#!/bin/bash
# Replaces SidebarLayout wrapper with a standalone layout in src/app/scanner/page.tsx

sed -i '' 's/import SidebarLayout from '\''@\/components\/SidebarLayout'\'';//g' src/app/scanner/page.tsx
sed -i '' 's/import AvatarMenu from '\''@\/components\/AvatarMenu'\'';//g' src/app/scanner/page.tsx
sed -i '' 's/import Breadcrumbs from '\''@\/components\/Breadcrumbs'\'';//g' src/app/scanner/page.tsx

# Create an ed script to replace the return statement block
ed -s src/app/scanner/page.tsx << 'EDEOF'
/return (/
.,$d
a
  return (
    <div className="min-h-screen bg-[#F9F9F7] font-sans text-foreground">
      ...
      <header className="bg-white border-b border-gray-200 px-4 md:px-8 py-4 flex justify-between items-center sticky top-0 z-50 shadow-sm">
        ...
        {/* Step Indicators */}
        <div className="flex items-center gap-1 sm:gap-3 text-[10px] sm:text-xs font-medium">
